...
function spread(totals, verdicts) {
  ...
  const bars = Object.entries(totals)
    .sort((a, b) => b[1] - a[1])
    .map(([name, count]) => {
      const hue = verdicts[name]?.hue ?? 210;
      const width = ((count / sum) * 100).toFixed(2);
      return `<div title="${escapeHtml(`${name} ${count}`)}" style="width:${width}%;background:hsl(${hue} 62% 52%)"></div>`;
    })
    .join('');
...
function legend(totals, verdicts) {
  const items = Object.entries(totals)
    ...
    .join('');
  return `<div class="gt-legend" style="margin-bottom:20px">${items}</div>`;
}
