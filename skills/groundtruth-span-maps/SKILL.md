...
nothing else close, `npx @highnessatharva/groundtruth check --fix-matches` applies it for you.
...
```bash
npx @highnessatharva/groundtruth check <path>          # blocking. exit 1 on a blocking finding
npx @highnessatharva/groundtruth check --json          # the machine contract, stdout only
npx @highnessatharva/groundtruth draft <path> --write  # scaffold a map, all TODO
npx @highnessatharva/groundtruth draft <path> --update # add new candidates, keep existing spans
npx @highnessatharva/groundtruth explain <rule>        # why a rule exists and what it measures
npx @highnessatharva/groundtruth resolve               # the only command that touches the network
```
